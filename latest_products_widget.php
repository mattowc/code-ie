...
class WP_Widget_Latest_Products extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ( $title )
			echo '<div class="headingblock">' . $title . '</div>';
...
function wpsc_latest_product( $args = null, $instance ) {
	...
	if ( count( $latest_products ) > 0 ) {
		...
		foreach ( $latest_products as $latest_product ) {
			$output .= '<li class="wpsc-latest-product" style="margin-bottom: 5px;">';
			// Thumbnails, if required
			if ($image) {
				$output .= '<div class="item_image">';
				$output .= '<a style="color: #EEE; text-transform: uppercase; padding-top: 5px;" href="' . wpsc_product_url( $latest_product->ID, null ) . '">';
			...
			// Link
			$output .= '<a href="' . wpsc_product_url( $latest_product->ID, null ) . '" class="wpsc-product-title" style="color: #EEE; text-transform: uppercase; padding-top: 5px;">'.stripslashes( $latest_product->post_title ).'</a><br />';
			//$output .= '<span style="color:lime; padding-top: 5px; font-size: 16px; font-weight:bold;">' . wpsc_the_product_price( ) . '</span></a>';
			$output .= '</li>';
		}
		$output .= "</ul>";
	}
