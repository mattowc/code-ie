...
if(wpsc_cart_item_count() < 1) :
   _e('Oops, there is nothing in your cart.', 'tokokoo') . "<a href=".get_option("product_list_url").">" . __('Please visit our shop', 'tokokoo') . "</a>";
   return;
endif;
?>
<div id="checkout_page_container">
	
	<div class="checkout_page_container_inside">
	
	<header>
	
		<h3><?php _e('Please review your order', 'tokokoo'); ?></h3>
	
		<a href="<?php echo get_option("product_list_url"); ?>" class="shopping-page" title="Continue Shopping" style="color: lime;">Continue Shopping</a>
		
	</header>
<br />	
<table class="checkout_cart" border="1">

   <tr class="header" style="font-size:14px;font-weight:bold;text-align:left;">
      <th colspan="2" ><?php _e('Product', 'tokokoo'); ?></th>
      <th><?php _e('Quantity', 'tokokoo'); ?></th>
      <th><?php _e('Price', 'tokokoo'); ?></th>
      <th><?php _e('Total', 'tokokoo'); ?></th>
      <th><?php _e('Remove', 'tokokoo'); ?></th>
   </tr>
      ...
      <tr class="product_row product_row_<?php echo wpsc_the_cart_item_key(); ?> <?php echo $alt_class;?>">
         ...
         <td class="wpsc_product_quantity wpsc_product_quantity_<?php echo wpsc_the_cart_item_key(); ?>">
            <form action="<?php echo get_option('shopping_cart_url'); ?>" method="post" class="adjustform qty">
               ...
               <input type="submit" value="<?php _e('+', 'tokokoo'); ?>" name="submit" />
            </form>
         </td>
         ...
         <td class="wpsc_product_remove wpsc_product_remove_<?php echo wpsc_the_cart_item_key(); ?>">
            <form action="<?php echo get_option('shopping_cart_url'); ?>" method="post" class="adjustform remove">
               ...
               <input type="hidden" name="key" value="<?php echo wpsc_the_cart_item_key(); ?>" />
               <input type="hidden" name="wpsc_update_quantity" value="true" />
               <input type="submit" value="<?php _e('Remove', 'tokokoo'); ?>" name="submit" />
            </form>
         </td>
      </tr>
   <?php endwhile; ?>
   ...
   <div id="wpsc_shopping_cart_left"><!-- Start #wpsc_shopping_cart_left -->
  ...
  <?php if(wpsc_uses_shipping()): ?>
   ...
   <?php if(wpsc_uses_coupons()): ?>
      ...
      <?php if(wpsc_coupons_error()): ?>
         <h4 class="coupons-error"><?php _e('Coupon is not valid.', 'tokokoo'); ?></h4>
      <?php endif; ?>
      ...
      <div class="wpsc_coupon">
        ...
        <span>
            <form  method="post" action="<?php echo get_option('shopping_cart_url'); ?>">
               ...
               <input type="submit" value="<?php _e('Update', 'tokokoo') ?>" />
            </form>
        </span>
      </div>
   ...
   <?php if(wpsc_uses_shipping()) : ?>
      <p><?php _e('Calculate Shipping Price', 'tokokoo'); ?></p>
      <table class="productcart">
         <tr class="wpsc_shipping_info">
            <td colspan="5">
               <span><?php _e('Please choose a country below to calculate your shipping costs', 'tokokoo'); ?></span>
            </td>
         </tr>
            ...
            <?php if (wpsc_have_valid_shipping_zipcode()) : ?>
                  <tr class='wpsc_update_location'>
                     <td colspan='5' class='shipping_error' >
                        <?php _e('Please provide a Zipcode and click Calculate in order to continue.', 'tokokoo'); ?>
                     </td>
                  </tr>
            <?php else: ?>
               <tr class='wpsc_update_location_error'>
                  <td colspan='5' class='shipping_error' >
                     <?php _e('Sorry, online ordering is unavailable to this destination and/or weight. Please double check your destination details.', 'tokokoo'); ?>
                  </td>
               </tr>
            <?php endif; ?>
         <?php endif; ?>
         ...
         <?php if (wpsc_have_morethanone_shipping_quote()) :?>
            <?php while (wpsc_have_shipping_methods()) : wpsc_the_shipping_method(); ?>
                  ...
                  <tr class='wpsc_shipping_header'><td class='shipping_header' colspan='5'><?php echo wpsc_shipping_method_name().__(' - Choose a Shipping Rate', 'tokokoo'); ?> </td></tr>
	...
	<?php if(wpsc_uses_shipping()) : ?>
	      <tr>
	         <td class='wpsc_total_price_and_shipping'colspan='2'>
	            <h4><?php _e('Review and purchase','tokokoo'); ?></h4>
	         </td>
	      </tr>
	
	      <tr class="total_price total_shipping">
	         <td class='wpsc_totals'>
	            <?php _e('Total Shipping', 'tokokoo'); ?>:
	         </td>
	         <td class='wpsc_totals_amount'>
   ...
   </div><!-- End #wpsc_shopping_cart_left -->
   
</div>
   
    <div id="wpsc_shopping_cart_right"><!-- Start #wpsc_shopping_cart_right -->
	
		<div id="wpsc_shopping_cart_right_inside">
   ...
   <?php do_action('wpsc_before_form_of_shopping_cart'); ?>
	...
	<?php if ( wpsc_show_user_login_form() && !is_user_logged_in() ): ?>
			...
			<div class="wpsc_registration_form_wrapper">
				
				<p><?php _e('You must sign in or register with us to continue with your purchase', 'tokokoo');?></p>
				
				<fieldset class='wpsc_registration_form'>
					<h2><?php _e( 'Sign in', 'tokokoo' ); ?></h2>
					
					<?php
					$args = array( 'remember' => false );
					wp_login_form( $args );
					?>
					<div class="wpsc_signup_text"><a href="#" class="signup"><?php _e('Sign Up', 'tokokoo');?></a></div>
				</fieldset>
			</div>
	<?php endif; ?>	
    ...
    <?php if(wpsc_show_user_login_form()):
		...
		<div class="wpsc_signup_form">
			
	        <fieldset class='wpsc_registration_form wpsc_right_registration'>
	        	<h2><?php _e('Join up now', 'tokokoo');?></h2>
	      
				<label><?php _e('Username', 'tokokoo'); ?>:</label>
				<input type="text" name="log" id="log" value="" size="20"/><br/>
				
				<label><?php _e('Password', 'tokokoo'); ?>:</label>
				<input type="password" name="pwd" id="pwd" value="" size="20" /><br />
				
				<label><?php _e('E-mail', 'tokokoo'); ?>:</label>
	            <input type="text" name="user_email" id="user_email" value="<?php echo attribute_escape(stripslashes($user_email)); ?>" size="20" /><br />
	            
	            <div class="wpsc_signup_text"><?php _e('Signing up is free and easy! please fill out your details your registration will happen automatically as you checkout. Don\'t forget to use your details to login with next time!', 'tokokoo');?></div>
	        </fieldset>
        ...
        <?php if(wpsc_checkout_form_is_header() == true){
               ...
               <tr <?php echo wpsc_the_checkout_item_error_class();?>>
                  <td <?php wpsc_the_checkout_details_class(); ?> colspan='2'>
                     <h1><?php echo wpsc_checkout_form_name();?></h1>
                  </td>
               </tr>
               ...
               <tr class='same_as_shipping_row'>
                  ...
                  elseif(isset($_POST['submit']) && !isset($_POST['shippingSameBilling']))
                   ...
                   ?>
					<label for='shippingSameBilling'><?php _e('Same as billing address:','tokokoo'); ?></label>
					<input type='checkbox' value='true' name='shippingSameBilling' id='shippingSameBilling' <?php echo $checked; ?> />
					<br/><span id="shippingsameasbillingmessage"><?php _e('Your order will be shipped to the billing address', 'tokokoo'); ?></span>
                  </td>
               </tr>
               <?php endif;
...
?>
      ...
      <tr>
         <td><label for='how_find_us'><?php _e('How did you find us' , 'tokokoo'); ?></label></td>
         <td>
            <select name='how_find_us'>
               <option value='Word of Mouth'><?php _e('Word of mouth' , 'tokokoo'); ?></option>
               <option value='Advertisement'><?php _e('Advertising' , 'tokokoo'); ?></option>
               <option value='Internet'><?php _e('Internet' , 'tokokoo'); ?></option>
               <option value='Customer'><?php _e('Existing Customer' , 'tokokoo'); ?></option>
            </select>
         </td>
      </tr>
      <?php endif; ?>
      ...
      <?php if(wpsc_gateway_count() > 1): // if we have more than one gateway enabled, offer the user a choice ?>
         ...
         <td colspan='2' class='wpsc_gateway_container'>
            <h3><?php _e('Payment Type', 'tokokoo');?></h3>
	...
	<div class='wpsc_make_purchase'>
         <span>
            ...
            <?php endif; ?>
               ...
               <input type='submit' value='' name='submit' class='make_purchase wpsc_checkout_button' />
         </span>
	</div>

<div class='clear'></div>
</form>
</div><!-- End #wpsc_shopping_cart_right_inside -->
</div><!-- End #wpsc_shopping_cart_right -->
</div><!--close checkout_page_container_inside-->
</div><!--close checkout_page_container-->
<?php
do_action('wpsc_bottom_of_shopping_cart');

?>
</div><!-- Not sure what this ends, but it ends something :P -->
